...
extern NSString *const TAQNAsyncUdpSocketException;
extern NSString *const TAQNAsyncUdpSocketErrorDomain;

extern NSString *const TAQNAsyncUdpSocketQueueName;
extern NSString *const TAQNAsyncUdpSocketThreadName;

typedef NS_ERROR_ENUM(TAQNAsyncUdpSocketErrorDomain, TAQNAsyncUdpSocketError) {
	TAQNAsyncUdpSocketNoError = 0,          // Never used
	TAQNAsyncUdpSocketBadConfigError,       // Invalid configuration
	TAQNAsyncUdpSocketBadParamError,        // Invalid parameter was passed
	TAQNAsyncUdpSocketSendTimeoutError,     // A send operation timed out
	TAQNAsyncUdpSocketClosedError,          // The socket was closed
	TAQNAsyncUdpSocketOtherError,           // Description provided in userInfo
};
...
@class TAQNAsyncUdpSocket;

@protocol TAQNAsyncUdpSocketDelegate <NSObject>
@optional
...
**/
- (void)udpSocket:(TAQNAsyncUdpSocket *)sock didConnectToAddress:(NSData *)address;
...
**/
- (void)udpSocket:(TAQNAsyncUdpSocket *)sock didNotConnect:(NSError * _Nullable)error;

/**
 * Called when the datagram with the given tag has been sent.
**/
- (void)udpSocket:(TAQNAsyncUdpSocket *)sock didSendDataWithTag:(long)tag;

/**
 * Called if an error occurs while trying to send a datagram.
 * This could be due to a timeout, or something more serious such as the data being too large to fit in a sigle packet.
**/
- (void)udpSocket:(TAQNAsyncUdpSocket *)sock didNotSendDataWithTag:(long)tag dueToError:(NSError * _Nullable)error;

/**
 * Called when the socket has received the requested datagram.
**/
- (void)udpSocket:(TAQNAsyncUdpSocket *)sock didReceiveData:(NSData *)data
...
**/
- (void)udpSocketDidClose:(TAQNAsyncUdpSocket *)sock withError:(NSError  * _Nullable)error;
 ...
 * Example:
 * 
 * TAQNAsyncUdpSocketReceiveFilterBlock filter = ^BOOL (NSData *data, NSData *address, id *context) {
 * 
 ...
 * 
**/
typedef BOOL (^TAQNAsyncUdpSocketReceiveFilterBlock)(NSData *data, NSData *address, id __nullable * __nonnull context);
 ...
 *
**/
typedef BOOL (^TAQNAsyncUdpSocketSendFilterBlock)(NSData *data, NSData *address, long tag);


@interface TAQNAsyncUdpSocket : NSObject

/**
 * TAQNAsyncUdpSocket uses the standard delegate paradigm,
...
- (instancetype)initWithDelegate:(nullable id<TAQNAsyncUdpSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq;
- (instancetype)initWithDelegate:(nullable id<TAQNAsyncUdpSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq NS_DESIGNATED_INITIALIZER;

#pragma mark Configuration

- (nullable id<TAQNAsyncUdpSocketDelegate>)delegate;
- (void)setDelegate:(nullable id<TAQNAsyncUdpSocketDelegate>)delegate;
- (void)synchronouslySetDelegate:(nullable id<TAQNAsyncUdpSocketDelegate>)delegate;
...
- (void)getDelegate:(id<TAQNAsyncUdpSocketDelegate> __nullable * __nullable)delegatePtr delegateQueue:(dispatch_queue_t __nullable * __nullable)delegateQueuePtr;
- (void)setDelegate:(nullable id<TAQNAsyncUdpSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
- (void)synchronouslySetDelegate:(nullable id<TAQNAsyncUdpSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;

/**
 * By default, both IPv4 and IPv6 are enabled.
 * 
 * This means TAQNAsyncUdpSocket automatically supports both protocols,
 * and can send to IPv4 or IPv6 addresses,
 * as well as receive over IPv4 and IPv6.
 * 
 * For operations that require DNS resolution, TAQNAsyncUdpSocket supports both IPv4 and IPv6.
 * If a DNS lookup returns only IPv4 results, TAQNAsyncUdpSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, TAQNAsyncUdpSocket will automatically use IPv6.
 ...
 * This is due to the fact that TAQNAsyncUdpSocket does NOT copy the data.
 ...
 *    or the system tools to handle this aren't available (e.g. on a mobile device).
 * 
 * For more information about TAQNAsyncUdpSocketSendFilterBlock, see the documentation for its typedef.
...
**/
- (void)setSendFilter:(nullable TAQNAsyncUdpSocketSendFilterBlock)filterBlock withQueue:(nullable dispatch_queue_t)filterQueue;
...
**/
- (void)setSendFilter:(nullable TAQNAsyncUdpSocketSendFilterBlock)filterBlock
 ...
 * 
 * Important Note:
 * TAQNAsyncUdpSocket may be running in parallel with your code.
 * That is, your delegate is likely running on a separate thread/dispatch_queue.
 * When you invoke this method, TAQNAsyncUdpSocket may have already dispatched delegate methods to be invoked.
 ...
 * [udpSocket setReceiveFilter:filter withQueue:myParsingQueue];
 * 
 * For more information about TAQNAsyncUdpSocketReceiveFilterBlock, see the documentation for its typedef.
 * To remove a previously set filter, invoke this method and pass a nil filterBlock and NULL filterQueue.
 * 
 * Note: This method invokes setReceiveFilter:withQueue:isAsynchronous: (documented below),
 *       passing YES for the isAsynchronous parameter.
**/
- (void)setReceiveFilter:(nullable TAQNAsyncUdpSocketReceiveFilterBlock)filterBlock withQueue:(nullable dispatch_queue_t)filterQueue;
 ...
 * For example, you can't query properties on the socket.
**/
- (void)setReceiveFilter:(nullable TAQNAsyncUdpSocketReceiveFilterBlock)filterBlock
               withQueue:(nullable dispatch_queue_t)filterQueue
          isAsynchronous:(BOOL)isAsynchronous;
 ...
 * 
 * The TAQNAsyncUdpSocket instance may optionally be used again.
 ...
 * 
 * Generally TAQNAsyncUdpSocket doesn't use CFStream. (It uses the faster GCD API's.)
