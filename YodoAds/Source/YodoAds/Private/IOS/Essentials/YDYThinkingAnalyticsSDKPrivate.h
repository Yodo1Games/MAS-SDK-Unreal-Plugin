...
static char ydy_TD_AUTOTRACK_VIEW_ID;
static char ydy_ydy_TD_AUTOTRACK_VIEW_ID_APPID;
static char ydy_TD_AUTOTRACK_VIEW_IGNORE;
static char ydy_ydy_TD_AUTOTRACK_VIEW_IGNORE_APPID;
static char ydy_TD_AUTOTRACK_VIEW_PROPERTIES;
static char ydy_ydy_TD_AUTOTRACK_VIEW_PROPERTIES_APPID;
static char ydy_TD_AUTOTRACK_VIEW_DELEGATE;
...
@interface YDYThinkingAnalyticsSDK ()
...
@property (atomic, copy) NSDictionary*(^autoTrackCallback)(YDYThinkingAnalyticsAutoTrackEventType type, NSDictionary *properties);// 自动采集回调
...
@property (atomic, strong) YDYTTSqliteDataQueue *dataQueue;
@property (nonatomic, copy) YDYTTConfig *config;
...
- (instancetype)initLight:(NSString *)appid withServerURL:(NSString *)serverURL withConfig:(YDYTTConfig *)config;
- (void)autotrack:(NSString *)event properties:(NSDictionary *_Nullable)propertieDict withTime:(NSDate *_Nullable)date;
- (BOOL)isViewControllerIgnored:(UIViewController *)viewController;
- (BOOL)isAutoTrackEventTypeIgnored:(YDYThinkingAnalyticsAutoTrackEventType)eventType;
- (BOOL)isViewTypeIgnored:(Class)aClass;
- (void)retrievePersistedData;
+ (dispatch_queue_t)ydy_td_trackQueue;
+ (dispatch_queue_t)ydy_td_networkQueue;
...
@interface YDYTTEventModel ()

@property (nonatomic, copy) NSString *timeString;
@property (nonatomic, assign) double zoneOffset;
@property (nonatomic, assign) YDYTimeValueType timeValueType;
...
@interface YDYLightYDYThinkingAnalyticsSDK : YDYThinkingAnalyticsSDK

- (instancetype)initWithAPPID:(NSString *)appID withServerURL:(NSString *)serverURL withConfig:(YDYTTConfig *)config;
