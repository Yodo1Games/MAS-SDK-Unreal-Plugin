...
public class YodoAds {
	...
	public static native void OnRewardAdFailedToLoad(String error);
    ...
    public static native void OnInterstitialAdFailedToLoad(String error);
    ...
    @Keep
    public static Yodo1MasBannerAdView addBannerToRootView(Activity activity, int size) {
        // Get the root view of the activity
        ViewGroup rootView = (ViewGroup) activity.getWindow().getDecorView().getRootView();

        // Create and configure the banner ad
        Yodo1MasBannerAdView bannerAdView = createBannerView(activity);
        setBannerSize(bannerAdView, size);

        // Configure banner layout parameters
        FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(320, 50); // Adjust width/height if needed
        params.setMargins(0, 0, 0, 0); // Adjust margins if needed

        // Add the banner ad to the root view
	...
	@Keep
	public static void setRewardListener(Activity activity) {
		activity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                ...
                ad.setAdListener(new Yodo1MasRewardAdListener() {
                    ...
                    public void onRewardAdFailedToLoad(Yodo1MasRewardAd ad, @NonNull Yodo1MasError error) {
						OnRewardAdFailedToLoad(getErrorString(error));
                    }
	...
	@Keep
	public static void setInterstitialListener(Activity activity) {
		activity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                ...
                ad.setAdListener(new Yodo1MasInterstitialAdListener() {
                    ...
                    public void onInterstitialAdFailedToLoad(Yodo1MasInterstitialAd ad, @NonNull Yodo1MasError error) {
                        OnInterstitialAdFailedToLoad(getErrorString(error));
                    }
    ...
    @Keep
	public static void loadRewardedAd(Activity activity, String placement) {
		activity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                RewardAdConfig config = new RewardAdConfig();
    ...
    @Keep
	public static void loadInterstitialAd(Activity activity, String placement) {
		activity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                InterstitialAdConfig config = new InterstitialAdConfig();
